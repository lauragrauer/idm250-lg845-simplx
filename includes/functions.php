...
function count_skus() {
    global $connection;
    $result = $connection->query("SELECT COUNT(*) as count FROM skus");
...
function count_inventory($location) {
    global $connection;
    $stmt = $connection->prepare("SELECT COUNT(*) as count FROM inventory WHERE location = ?");
    $stmt->bind_param('s', $location);
    $stmt->execute();
    $result = $stmt->get_result();
    $row = $result->fetch_assoc();
...
function count_shipped_orders() {
    global $connection;
    $result = $connection->query("SELECT COUNT(*) as count FROM orders WHERE status = 'confirmed'");
    $row = $result->fetch_assoc();
    return $row['count'];
}
